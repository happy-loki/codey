...
主 ChatView 是单实例会话视图。resume 或新建线程切换前，先等待当前线程的活动 turn 被中断，再发送 `thread/unsubscribe`；这只解除当前 app-server 连接的订阅，不归档、不删除历史。这样旧线程进入空闲卸载流程后，会释放本地 rollout writer，避免同一 Codey 实例长期占用旧会话。
