import type { ExtendedProperties, IOpts, RendererAPI, ThemeStyles, ReadTimeResults } from '@md/shared/types'
...
function parseFrontMatterAndContent(markdownText: string): ParseResult {
  try {
    ...
    const readingTimeResult = calculateReadingTime(markdownContent)
    ...
    return {
      ...
      readingTime: calculateReadingTime(markdownText),
    }
  }
}
...
export function initRenderer(opts: IOpts): RendererAPI {
  ...
  const renderer: RendererObject = {
    ...
    code({ text, lang = `` }: Tokens.Code): string {
      ...
      const showMac = Boolean(opts.isMacCodeBlock)
      const macSpan = showMac
        ? `<span class="mac-sign" style="display:flex;align-items:center;gap:6px;padding:10px 14px 0;">${macCodeSvg}</span>`
        : ``
      const code = `<code class="language-${lang}" ${styles(`code`, `;padding:0.5em 1em 1em;display:block;`)}>${highlighted}</code>`
      return `<pre class="hljs code__pre" ${styles(`code_pre`, `;padding:0;`)}>${macSpan}${code}</pre>`
    },
